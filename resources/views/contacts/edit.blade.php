...
<div class="uk-text-justify uk-margin-remove-top" uk-grid>
    <div class="uk-width-expand@m">
        <div class="uk-card">
            ...
            <form class="uk-grid-small" action="{{route('contacto.update', $contacto->id)}}" method="POST" uk-grid>
                @csrf
                @method('PUT')
                <div class="uk-width-1-2@s">
                    <label class="uk-form-label">Telefone principal</label>
                <input class="uk-input" name="celular" type="tel" placeholder="Telefone principal" value="{{$contacto->celular}}" required>
                </div>
                <div class="uk-width-1-2@s">
                    <label class="uk-form-label">Contacto do whatsapp</label>
                    <input class="uk-input" type="tel" name="whatsapp" placeholder="Telefone alternativo" value="{{$contacto->whatsapp}}" required>
                </div>
                <div class="uk-width-1-2@s">
                    <label class="uk-form-label">Localização</label>
                    <textarea class="uk-textarea" rows="3" type="text" name="localizacao"
                        placeholder="Localização">{{$contacto->localizacao}}</textarea>
                </div>
                <div class="uk-width-1-2@s">
                    <label class="uk-form-label">Correio eletrónico</label>
                    <input class="uk-input" type="email" name="email" value="{{$contacto->email}}" placeholder="Correio eletrónico">
                </div>
