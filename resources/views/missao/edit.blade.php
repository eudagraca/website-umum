        ...
        <div class="uk-card">
            <form action="{{ route('missao.update', $valor->id) }}" method="POST">
                ...
                <fieldset class="uk-fieldset uk-flex-center">
                    ...
                    <div class="uk-margin"">
                            <textarea autocomplete="false" class="uk-textarea @error('descricao') uk-form-danger @enderror" rows="7" id="descricao"
                                name="descricao" placeholder="Descrição">{{ $valor->descricao }}</textarea>
                            @error('descricao')
                            <div class="uk-text-danger">{{ $message }}</div>
                            @enderror
                        <script>
